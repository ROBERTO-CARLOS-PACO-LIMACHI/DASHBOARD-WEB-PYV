...
export class NodeModel {
  ...
  static async addEvento(data){
    ...
    const mapEje=(eje=[],timestamp=[])=>{
      ...
      return eje.map((value,i)=>({value:2*(9.81)*Math.abs(value)/32768,timestamp:new Date(timestamp[i])}))
    }
    const mapEjez=(eje=[],timestamp=[])=>{
      console.log("eje a convertir: ",eje)
      return eje.map((value,i)=>({value:Math.abs(2*(9.81)*Math.abs(value)/32768-9.81),timestamp:new Date(timestamp[i])}))
    }
    const resultado={
    ...
    const vector_suma=Object.keys(datos.eje_x).map(k=>({
      ...
      timestamp:new Date(timestamp[k])
   }))
